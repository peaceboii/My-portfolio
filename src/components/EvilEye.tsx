...
void main() {
  ...
  float eyeIntensity = clamp(max(innerRing + innerEye, outerEyeGlow + outerBgGlow) - pupil, 0.0, 3.0);
  vec3 color = uEyeColor * uIntensity * eyeIntensity;
  float alpha = clamp(eyeIntensity, 0.0, 1.0);

  gl_FragColor = vec4(color, alpha);
}
`;

const EvilEye: React.FC<EvilEyeProps> = ({
  eyeColor = '#FF6F37',
  intensity = 1.5,
  pupilSize = 0.6,
  irisWidth = 0.25,
