...
const SelectedWorks: React.FC = () => {
  // Resolve the URLs with BASE_URL to prevent 404s
  const getAssetUrl = (path: string) => {
    return `${import.meta.env.BASE_URL}${path.replace(/^\//, '')}`;
  };

  return (
    <Section3D id="work" className="bg-bg py-24 md:py-32">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div className="max-w-xl">
            ...
            <h2 className="text-4xl md:text-5xl lg:text-6xl text-text-primary mb-6">
              <Text3D text="Featured" delay={0.1} />{' '}
              <span className="font-display italic inline-block">
                <Text3D text="projects" delay={0.25} />
              </span>
            </h2>
            <Container3D delay={0.3} duration={0.8} tiltOnHover={false} rotateXStart={10} yStart={20}>
          ...
          <Container3D delay={0.4} duration={0.8} tiltOnHover={false} rotateXStart={10} yStart={20}>
            <a 
                  ...
                  <div className="flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
                    <div className="relative p-[1px] rounded-full accent-gradient">
                      <div className="bg-white px-5 py-2 rounded-full">
                        <span className="text-bg text-xs font-semibold">View Code</span>
                      </div>
                    </div>
                  </div>
                </div>
