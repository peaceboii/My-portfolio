...
async function loadFontFromStylesheet(url: string) {
  ...
  for (const block of faceBlocks) {
    ...
    // Type casting descriptors as any to allow dynamic descriptors setting
    const desc = descriptors as any;
    if (weightMatch) desc.weight = weightMatch[1].trim();
    if (styleMatch) desc.style = styleMatch[1].trim();
    if (rangeMatch) desc.unicodeRange = rangeMatch[1].trim();
    fontFaces.push(new FontFace(family, `url(${urlMatch[1]})`, desc));
  }
